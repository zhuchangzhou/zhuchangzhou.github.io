...
$(function() {
	...
	var loginForm=document.forms['loginForm'],
		...
		//获取cookie数据并解析json数据开始	
		//debugger
		var getcook = getCookie();
		var cook = unescape(getcook);
		let cookieArr = cook.split(";");
		var strrr = cook.split(";");
		let obj = {}
		cookieArr.forEach((i) => {
			let arr = i.split("=");
			obj[arr[0]] = arr[1];
    ...
    //判断是否包含参数
		for(var i=0;i<strrr.length;i++){
				if (strrr[i].indexOf("loginId")>-1) {
					continue;
				}
				if (strrr[i].indexOf("loginType")>-1) {
					continue;
				}
				if (strrr[i].indexOf("login_name")>-1) {
					continue;
				}
				if (strrr[i].indexOf("password")>-1) {
					continue;
				}

				kechen_course += '<div class="sousuo_center_one">' + strrr[i].split("=")[0] + '</div>';
		}
		//获取cookie数据并解析json结束
		kechen_course += '</div>' +
			'<div class="resousuo">' +
		...
		success: function(result) {
			...
			if(data!=null){
				...
				if(itemList!=null){
						...
						window.location.href = '../yemian/kechen.html?courseTypeId=' + courseTypeId;
				}else{
